...
pub fn process_results(raw_results: RawResults) -> Response<FinalResults> {
    ...
    let first_time = raw_results.events.iter().filter(|e| e.correct.is_some()).map(|e| e.time).min().unwrap_or(Duration::ZERO);
    let last_time = raw_results.events.iter().filter(|e| e.correct.is_some()).map(|e| e.time).max().unwrap_or(Duration::ZERO);
